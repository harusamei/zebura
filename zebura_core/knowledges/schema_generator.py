...
class Scanner:
    ...
    def gen_schema(self, file_path):
        ...
        if project in sheet_names:
            schema = self.gen_project(file_path)
        else:
        ...
        output_path = os.path.join(os.path.dirname(file_path), f"{project_code}_meta.json")
        ...
        output_path = os.path.join(os.path.dirname(file_path), f"{project_code}_gcases.json")
